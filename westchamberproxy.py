...
import os, re, socket, struct, threading, traceback, sys, select, urlparse, signal, urllib, urllib2, time, hashlib, binascii, zlib, httplib, errno, string, logging, random, ssl, heapq, fnmatch, thread
import DNS
...
class PacServerHandler(BaseHTTPRequestHandler):

    def do_GET(self):
        filename = os.path.join(os.path.dirname(__file__), "flora_pac.pac")
        if self.path != '/'+"flora_pac.pac" or not os.path.isfile(filename):
...
class ProxyAndPacHandler(ProxyHandler, PacServerHandler):
    def do_GET(self):
        if self.path == '/'+"flora_pac.pac":
            PacServerHandler.do_GET(self)
        else:
            ProxyHandler.do_METHOD_Tunnel(self)
...
def start():
    ...
    print "Pac Server:  http://127.0.0.1:%d/flora_pac.pac\n" %(gConfig["PAC_PORT"])
